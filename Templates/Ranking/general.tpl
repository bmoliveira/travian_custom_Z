...
\** --------------------------------------------------- **/
   ...
   $tribe1 = $database->mysql_query_adapter("SELECT * FROM ".TB_PREFIX."users WHERE tribe = 1");
   $tribe2 = $database->mysql_query_adapter("SELECT * FROM ".TB_PREFIX."users WHERE tribe = 2");
   $tribe3 = $database->mysql_query_adapter("SELECT * FROM ".TB_PREFIX."users WHERE tribe = 3");
   $tribes = array(mysql_num_rows($tribe1), mysql_num_rows($tribe2), mysql_num_rows($tribe3));
   $users = mysql_num_rows($database->mysql_query_adapter("SELECT * FROM " . TB_PREFIX . "users WHERE tribe!=0 AND tribe!=4 AND tribe!=5")); ?>
    ...
    <table cellpadding="1" cellspacing="1" id="world_player" class="world">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><?php
                   $active = mysql_num_rows($database->mysql_query_adapter("SELECT * FROM ".TB_PREFIX."users WHERE ".time()."-timestamp < (3600*24) AND tribe!=0 AND tribe!=4 AND tribe!=5"));
                   echo $active; ?></td>
            </tr>
            ...
            <tr>
                ...
                <td><?php
                   $online = mysql_num_rows($database->mysql_query_adapter("SELECT * FROM ".TB_PREFIX."users WHERE ".time()."-timestamp < (60*10) AND tribe!=0 AND tribe!=4 AND tribe!=5"));
                   echo $online; ?></td>
            </tr>
        </tbody>
    </table>
