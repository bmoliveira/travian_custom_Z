...
$sql = $database->mysql_query_adapter("SELECT * FROM ".TB_PREFIX."farmlist WHERE owner = $session->uid ORDER BY wref = $village->wid DESC");
...
<?php
$sql2 = $database->mysql_query_adapter("SELECT * FROM ".TB_PREFIX."raidlist WHERE lid = $lid ORDER BY distance ASC");
...
$getnotice = $database->mysql_query_adapter("SELECT * FROM ".TB_PREFIX."ndata WHERE $limits AND toWref = ".$towref." AND uid = ".$session->uid." ORDER BY time DESC Limit 1");
...
<script type="text/javascript">
        window.addEvent('domready', function()
        {
            Travian.Game.RaidList.setData({
<?php
$result = $database->mysql_query_adapter('SELECT * FROM '.TB_PREFIX.'farmlist WHERE wref = '.$village->wid.'');
...
?>
                "<?php echo $lid; ?>":{
                    ...
                    "slots":{<?php 
$result3 = $database->mysql_query_adapter('SELECT * FROM '.TB_PREFIX.'raidlist WHERE lid = '.$lid.'');
