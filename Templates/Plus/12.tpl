...
if($golds['b4'] < time()) {
$database->mysql_query_adapter("UPDATE ".TB_PREFIX."users set b4 = '".(time()+PLUS_PRODUCTION)."' where `id`='".$session->uid."'") or die(mysql_error());
} else {
$database->mysql_query_adapter("UPDATE ".TB_PREFIX."users set b4 = '".($golds['b4']+PLUS_PRODUCTION)."' where `id`='".$session->uid."'") or die(mysql_error());
}
...
$done1 = "+25% Production: Crop";
    ...
    $database->mysql_query_adapter("INSERT INTO ".TB_PREFIX."gold_fin_log VALUES ('".(mysql_num_rows($goldlog)+1)."', '".$village->wid."', '+25%  Production: Crop')") or die(mysql_error());

} else {
$done1 = "nothing has been done";
    $database->mysql_query_adapter("INSERT INTO ".TB_PREFIX."gold_fin_log VALUES ('".(mysql_num_rows($goldlog)+1)."', '".$village->wid."', 'Failed +25%  Production: Crop')") or die(mysql_error());
