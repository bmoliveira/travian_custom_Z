...
if($golds['b3'] < time()) {
$database->mysql_query_adapter("UPDATE ".TB_PREFIX."users set b3 = '".(time()+PLUS_PRODUCTION)."' where `id`='".$session->uid."'") or die(mysql_error());
} else {
$database->mysql_query_adapter("UPDATE ".TB_PREFIX."users set b3 = '".($golds['b3']+PLUS_PRODUCTION)."' where `id`='".$session->uid."'") or die(mysql_error());
}
...
$done1 = "+25% Production: Iron";
    ...
    $database->mysql_query_adapter("INSERT INTO ".TB_PREFIX."gold_fin_log VALUES ('".(mysql_num_rows($goldlog)+1)."', '".$village->wid."', '+25%  Production: Iron')") or die(mysql_error());

} else {
$done1 = "nothing has been done";
    $database->mysql_query_adapter("INSERT INTO ".TB_PREFIX."gold_fin_log VALUES ('".(mysql_num_rows($goldlog)+1)."', '".$village->wid."', 'Failed +25%  Production: Iron')") or die(mysql_error());
