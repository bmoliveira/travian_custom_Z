...
EOT;
    ...
    $account = $database->escape_string_query($_POST['username']);
    $reward = $database->escape_string_query($_POST['reward']);
    $valid=TRUE;
...
/////////////////////////////////////////////////////////
    ...
    $accountCheck = $database->mysql_query_adapter("SELECT * FROM ".TB_PREFIX."users WHERE `id`='".$session->uid."'") or die(mysql_error());
    ...
    $plusCheck = $database->mysql_query_adapter("SELECT * FROM ".TB_PREFIX."users WHERE `id`='".$session->uid."'") or die(mysql_error());
...
echo' Please select the option you wish to activate or extend.<br>';
    ...
    if(mysql_num_rows($plusCheck) > 0){ 
        if($time > $pluss[$key] ){
            $editplus = $database->mysql_query_adapter("UPDATE ".TB_PREFIX."users SET `{$key}`= `{$key}` + ('".$time."'+'".$plusTime."'),  `gold` =  `gold` - {$gldz}   WHERE `id`='".$session->uid."'") or die(mysql_error());
        ...
        if($time < $pluss[$key]){
            $editplus = $database->mysql_query_adapter("UPDATE ".TB_PREFIX."users SET `{$key}`= `{$key}` +'".$plusTime."',  `gold` =  `gold` - {$gldz}  WHERE `id`='".$session->uid."'") or die(mysql_error());
            echo "<META HTTP-EQUIV=Refresh CONTENT=\"2; {$url}\" ><br /><br /><div align=center><font color=green size=4><b> Your Status has been updated!</b></font></div>";
       
       }
    }
else{
        $insertplus = $database->mysql_query_adapter("INSERT INTO ".TB_PREFIX."users (`username`,`{$key}`, `gold`) VALUES ('".$session->username."', ('".$time."'+'".$plusTime."'),`gold` - {$gldz})") or die(mysql_error());
