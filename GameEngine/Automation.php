...
class Automation {
	...
	private $bountyOpop = 1;
		...
		$q = $database->mysql_query_adapter("SELECT vref FROM ".TB_PREFIX."fdata WHERE f99 = '100' and f99t = '40'");
		   ...
		   if (($level+1) == $indi['level']){
			...
			if($database->query($q)) {
				...
				$this->procClimbers($database->getVillageField($indi['wid'],'owner'));
					...
					$database->mysql_query_adapter("TRUNCATE ".TB_PREFIX."bdata");
					}
	...
	private function startNatarAttack($level, $vid, $time) {
		...
		$query = $database->mysql_query_adapter('SELECT `wref` FROM `' . TB_PREFIX . 'vdata` WHERE `owner` = 3 and `capital` = 1 LIMIT 1') or die(mysql_error());
		...
		// -.-
		$database->mysql_query_adapter('INSERT INTO `' . TB_PREFIX . 'ww_attacks` (`vid`, `attack_time`) VALUES (' . $vid . ', ' . $endtime . ')');
		$database->mysql_query_adapter('INSERT INTO `' . TB_PREFIX . 'ww_attacks` (`vid`, `attack_time`) VALUES (' . $vid . ', ' . ($endtime + 1) . ')');
	...
	private function checkWWAttacks() {
		$query = $database->mysql_query_adapter('SELECT * FROM `' . TB_PREFIX . 'ww_attacks` WHERE `attack_time` <= ' . time());
		while ($row = mysql_fetch_assoc($query))
		{
			// delete the attack
			$query3 = $database->mysql_query_adapter('DELETE FROM `' . TB_PREFIX . 'ww_attacks` WHERE `vid` = ' . $row['vid'] . ' AND `attack_time` = ' . $row['attack_time']);
		}
	}
		   ...
		   if ($type=='3')
{
	if (($data['t8']-$traped8)>0)
	{
		...
		else
		{
			...
			if ($data['ctar2']==0)
			{
				$bdo2=$database->mysql_query_adapter("select * from " . TB_PREFIX . "fdata where vref = $basearray");
			        ...
			        if ($battlepart[4]>$battlepart[3])
				{
					...
					if ($tbgid==10 || $tbgid==38) {
						$tsql=$database->mysql_query_adapter("select `maxstore`,`maxcrop` from ".TB_PREFIX."vdata where wref=".$data['to']."");
...
$wallimg = "<img src=\"".GP_LOCATE."img/g/g3".$targettribe."Icon.gif\" height=\"20\" width=\"15\" alt=\"Wall\" title=\"Wall\" />";
		...
		if(!empty($harray)){
			foreach($harray as $hdata){
				...
				for($i = $herolevel+1; $i < 100; $i++){
					...
					$database->mysql_query_adapter("UPDATE " . TB_PREFIX ."hero SET level = $i WHERE heroid = '".$hdata['heroid']."'");
					if($i < 99){
					$database->mysql_query_adapter("UPDATE " . TB_PREFIX ."hero SET points = points + 5 WHERE heroid = '".$hdata['heroid']."'");
					}
					}
				}
					$villunits = $database->getUnit($hdata['wref']);
					if($villunits['hero'] == 0 && $hdata['trainingtime'] < time() && $hdata['inrevive'] == 1){
					$database->mysql_query_adapter("UPDATE " . TB_PREFIX . "units SET hero = 1 WHERE vref = ".$hdata['wref']."");
					$database->mysql_query_adapter("UPDATE ".TB_PREFIX."hero SET `dead` = '0', `inrevive` = '0', `health` = '100', `lastupdate` = ".$hdata['trainingtime']." WHERE `uid` = '".$hdata['uid']."'");
					}
					if($villunits['hero'] == 0 && $hdata['trainingtime'] < time() && $hdata['intraining'] == 1){
					$database->mysql_query_adapter("UPDATE " . TB_PREFIX . "units SET hero = 1 WHERE vref = ".$hdata['wref']."");
					$database->mysql_query_adapter("UPDATE ".TB_PREFIX."hero SET `intraining` = '0', `lastupdate` = ".$hdata['trainingtime']." WHERE `uid` = '".$hdata['uid']."'");
					}
			}
		}
		if(file_exists("GameEngine/Prevention/updatehero.txt")) {
 ...
 // by SlimShady95, aka Manuel Mannhardt < manuel_mannhardt@web.de > UPDATED FROM songeriux < haroldas.snei@gmail.com >
	private function updateStore() {
		...
		$result = $database->mysql_query_adapter('SELECT * FROM `' . TB_PREFIX . 'fdata`');
		while ($row = mysql_fetch_assoc($result))
		{
			...
			$database->mysql_query_adapter('UPDATE `' . TB_PREFIX . 'vdata` SET `maxstore` = ' . $ress . ', `maxcrop` = ' . $crop . ' WHERE `wref` = ' . $row['vref']) or die(mysql_error());
		}
	}
	...
	private function checkInvitedPlayes() {
		...
		$numusers = $database->mysql_query_adapter("SELECT * FROM ".TB_PREFIX."users WHERE id = ".$user['invited']);
	...
	private function updateGeneralAttack() {
		...
		if(time() - (86400*8) > $general['time']){
			$database->mysql_query_adapter("UPDATE ".TB_PREFIX."general SET shown = 0 WHERE id = ".$general['id']."");
		}
		}
	}
	...
	private function checkBan() {
		...
		foreach($array as $banlist) {
			$database->mysql_query_adapter("UPDATE ".TB_PREFIX."banlist SET active = 0 WHERE id = ".$banlist['id']."");
			$database->mysql_query_adapter("UPDATE ".TB_PREFIX."users SET access = 2 WHERE id = ".$banlist['uid']."");
		}
	}
	...
	private function checkReviveHero(){
		...
		if ($herodata[0]['dead']==1){
			$database->mysql_query_adapter("UPDATE " . TB_PREFIX . "units SET hero = 0 WHERE vref = ".$session->villages[0]."");
		}
    	if($herodata[0]['trainingtime'] <= time()) {
        		if($herodata[0]['trainingtime'] != 0) {
        			if($herodata[0]['dead'] == 0) {
        				$database->mysql_query_adapter("UPDATE " . TB_PREFIX . "hero SET trainingtime = '0' WHERE uid = " . $session->uid . "");
						$database->mysql_query_adapter("UPDATE " . TB_PREFIX . "units SET hero = 1 WHERE vref = ".$session->villages[0]."");
 			       }
        		}
        	}
	}
	...
	************************************************/
    ...
    //Attackers of the week
      $result = $database->mysql_query_adapter("SELECT * FROM ".TB_PREFIX."users ORDER BY ap DESC, id DESC Limit 10");
      $i=0;   
    while($row = mysql_fetch_array($result)){
    $i++;   
    $img="t2_".($i)."";
    $quer="insert into ".TB_PREFIX."medal(userid, categorie, plaats, week, points, img) values('".$row['id']."', '1', '".($i)."', '".$week."', '".$row['ap']."', '".$img."')";
    $resul=$database->mysql_query_adapter($quer);    
    }

    //Defender of the week
      $result = $database->mysql_query_adapter("SELECT * FROM ".TB_PREFIX."users ORDER BY dp DESC, id DESC Limit 10");
      $i=0;   
    while($row = mysql_fetch_array($result)){
    $i++;   
    $img="t3_".($i)."";
    ...
    //Climbers of the week
      $result = $database->mysql_query_adapter("SELECT * FROM ".TB_PREFIX."users ORDER BY Rc DESC, id DESC Limit 10");
      $i=0;   
    while($row = mysql_fetch_array($result)){
    $i++;   
    $img="t1_".($i)."";
    ...
    $resul=$database->mysql_query_adapter($quer);    
      ...
      $result = $database->mysql_query_adapter("SELECT * FROM ".TB_PREFIX."users ORDER BY clp DESC Limit 10");
      $i=0;     
    while($row = mysql_fetch_array($result)){
      $i++;    
    $img="t6_".($i)."";
    ...
    //Robbers of the week
      $result = $database->mysql_query_adapter("SELECT * FROM ".TB_PREFIX."users ORDER BY RR DESC, id DESC Limit 10");
      $i=0;   
    while($row = mysql_fetch_array($result)){
    $i++;   
    $img="t4_".($i)."";
    ...
    //Top10 attackers
      ...
      while($row = mysql_fetch_array($result)){
       ...
       //Top 10 defenders
      $result2 = $database->mysql_query_adapter("SELECT * FROM ".TB_PREFIX."users ORDER BY dp DESC, id DESC Limit 10");
      while($row2 = mysql_fetch_array($result2)){
        if($row['id']==$row2['id']){

        $query3="SELECT count(*) FROM ".TB_PREFIX."medal WHERE userid='".$row['id']."' AND categorie = 5";
        $result3=$database->mysql_query_adapter($query3);
    ...
    //top10 attackers
      ...
      while($row = mysql_fetch_array($result)){ 
        ...
        $result1=$database->mysql_query_adapter($query1);
    ...
    //je staat voor 3e / 5e / 10e keer in de top 3 klimmers 
      ...
      $result = $database->mysql_query_adapter("SELECT * FROM ".TB_PREFIX."users ORDER BY clp DESC, id DESC Limit 10");
      while($row = mysql_fetch_array($result)){ 
    ...
    //Put all true dens to 0
     ...
     $database->mysql_query_adapter("UPDATE ".TB_PREFIX."users SET ap=0, dp=0,Rc=0,clp=0, RR=0 WHERE id = ".$row[0]."");
    }

    //Start alliance Medals wooot

    //Aanvallers v/d Week
    $result = $database->mysql_query_adapter("SELECT * FROM ".TB_PREFIX."alidata ORDER BY ap DESC, id DESC Limit 10");
    ...
    //Verdediger v/d Week
    $result = $database->mysql_query_adapter("SELECT * FROM ".TB_PREFIX."alidata ORDER BY dp DESC Limit 10");
    $i=0;     while($row = mysql_fetch_array($result)){
    $i++;    $img="a3_".($i)."";
    ...
    $result = $database->mysql_query_adapter("SELECT * FROM ".TB_PREFIX."alidata ORDER BY RR DESC, id DESC Limit 10");
    $i=0;     while($row = mysql_fetch_array($result)){
    $i++;    $img="a4_".($i)."";
    ...
    //Rank climbers of the week
    $result = $database->mysql_query_adapter("SELECT * FROM ".TB_PREFIX."alidata ORDER BY clp DESC Limit 10");
    $i=0;     while($row = mysql_fetch_array($result)){
    $i++;    $img="a1_".($i)."";
    ...
    while($row = mysql_fetch_array($result)){
      ...
      //Pak de top10 verdedigers
      $result2 = $database->mysql_query_adapter("SELECT * FROM ".TB_PREFIX."alidata ORDER BY dp DESC, id DESC Limit 10");
      while($row2 = mysql_fetch_array($result2)){
        if($row['id']==$row2['id']){
      ...
      $result=$database->mysql_query_adapter($query);
      for ($i=0; $row=mysql_fetch_row($result); $i++){
      $database->mysql_query_adapter("UPDATE ".TB_PREFIX."alidata SET ap=0, dp=0, RR=0, clp=0 WHERE id = ".$row[0]."");
      }
	...
	************************************************/
	 ...
	 global $database;
		...
		$database->mysql_query_adapter("UPDATE ".TB_PREFIX."artefacts SET kind = $kind, bad_effect = $bad_effect, effect2 = $effect, lastupdate = $time WHERE id = ".$artefact['id']."");
		}
	}
}
