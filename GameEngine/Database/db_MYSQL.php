...
class MYSQL_DB {
	...
	function MYSQL_DB() {
		...
		$database->mysql_query_adapter("SET NAMES 'UTF8'");  //Fix utf8 phpmyadmin by gm4st3r
	}
	...
	function register($username, $password, $email, $tribe, $act) {
		...
		if($database->mysql_query_adapter($q, $this->connection)) {
	...
	function caststruc($user) {
		...
		$query = $database->mysql_query_adapter("SELECT * FROM ".TB_PREFIX."vdata WHERE owner = ".$user."");
		while($villaggi_array = mysql_fetch_array($query))

		//loop structure village
		$query1 = $database->mysql_query_adapter("SELECT * FROM ".TB_PREFIX."fdata WHERE vref = ".$villaggi_array['wref']."");
	...
	function removeMeSit($uid, $uid2) {
		...
		$database->mysql_query_adapter($q2, $this->connection);
	}
    ...
    function getUserField($ref, $field, $mode) {
        ...
        $result = $database->mysql_query_adapter($q, $this->connection) or die(mysql_error());
        if($result) {
            $dbarray = mysql_fetch_array($result);
	...
	function sitterLogin($username, $password) {
		...
		if($dbarray['sit1'] != 0) {
			...
			$result2 = $database->mysql_query_adapter($q2, $this->connection);
			$dbarray2 = mysql_fetch_array($result2);
		}
		if($dbarray['sit2'] != 0) {
				$q3 = "SELECT password FROM " . TB_PREFIX . "users where id = " . $dbarray['sit2'] . " and access != " . BANNED;
				$result3 = $database->mysql_query_adapter($q3, $this->connection);
	...
	*****************************************/
	...
	function updateActiveUser($username, $time) {
		...
		$exec1 = $database->mysql_query_adapter($q, $this->connection);
		$exec2 = $database->mysql_query_adapter($q2, $this->connection);
		if($exec1 && $exec2) {
			return true;
		} else {
			return false;
	...
	function addVillage($wid, $uid, $username, $capital) {
		...
		return $database->mysql_query_adapter($q, $this->connection) or die(mysql_error());
	}
	...
	$troops_o=0;
	$o_unit2=$database->mysql_query_adapter("select * from ".TB_PREFIX."units where `vref`='".$vref."'");
	...
	$o_unit2=$database->mysql_query_adapter("select * from ".TB_PREFIX."enforcement where `vref`='".$vref."'");
	...
	*****************************************/
	...
	function CountTopic($id) {
		...
		$results = $database->mysql_query_adapter($qs, $this->connection);
  ...
  function getVoteSum($topic) {
	...
	*************************/
    ...
    function DeleteCat($id) {
        ...
        $result = $database->mysql_query_adapter($q2, $this->connection);
        if (!empty($result)) {
            $array=$this->mysql_fetch_all($result);
            foreach($array as $ss) {
        ...
        $database->mysql_query_adapter($qs, $this->connection);
        return $database->mysql_query_adapter($q, $this->connection);
    }

	function DeleteSurvey($id) {
        $qs = "DELETE from " . TB_PREFIX . "forum_survey where topic = '$id'";
        return $database->mysql_query_adapter($qs, $this->connection);
    }
	...
	*****************************************/
	function deleteAlliance($aid) {
		$result = $database->mysql_query_adapter("SELECT * FROM " . TB_PREFIX . "users where alliance = $aid");
		$num_rows = mysql_num_rows($result);
		if($num_rows == 0) {
	...
	*****************************************/
	 ...
	 function modifyResource($vid, $wood, $clay, $iron, $crop, $mode) {
    	...
    	if(!$shit){
    		$q = "UPDATE " . TB_PREFIX . "vdata set wood = $dwood, clay = $dclay, iron = $diron, crop = $dcrop where wref = ".$vid;
    			return $database->mysql_query_adapter($q, $this->connection); }else{return false;}
   	}
	...
	function getFieldDistance($wid) {
        ...
        $array2 = mysql_fetch_array($database->mysql_query_adapter($q2));
        $vill = $array2['wref'];
        if(mysql_num_rows($database->mysql_query_adapter($q)) > 0){
	...
	***************************/
	...
	$x = "UPDATE " . TB_PREFIX . "users SET gold = gold - 2 WHERE id = ".$uid."";
		$database->mysql_query_adapter($x, $this->connection);
	...
	function addBuilding($wid, $field, $type, $loop, $time, $master, $level) {
		...
		$database->mysql_query_adapter($x, $this->connection) or die(mysql_error());
	...
	function removeBuilding($d) {
		...
		if($SameBuildCount > 0) {
			...
			$q1 = "UPDATE " . TB_PREFIX . "bdata SET level=level-1,timestamp=".$timestamp1." WHERE id=".$jobs[$jobMaster]['id']."";
				$database->mysql_query_adapter($q1, $this->connection);
			}else{
            ...
            if($jobs[$jobDeleted]['field'] >= 19) {
                ...
                $result = $database->mysql_query_adapter($x, $this->connection) or die(mysql_error());
	...
	function FinishWoodcutter($wid) {
		...
		$result2 = $database->mysql_query_adapter($q2);
	...
	***************************/
	...
	function Getowner($vid) {
		...
		$result1 = $database->mysql_query_adapter($s, $this->connection);
...
*/
	...
	function addPassword($uid, $npw, $cpw){
		...
		$database->mysql_query_adapter($q, $this->connection) or die(mysql_error());
	}
...
*****************************************/
   ...
   function getvacmodexy($wref){
	$q = "SELECT id,oasistype,occupied FROM " . TB_PREFIX . "wdata where id = $wref";
     ...
     $result1 = $database->mysql_query_adapter($q1, $this->connection);
